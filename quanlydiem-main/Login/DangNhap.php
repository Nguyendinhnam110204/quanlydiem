...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT idNguoiDung, TenDangNhap, MatKhau, VaiTro FROM NguoiDung WHERE TenDangNhap = ?");
    ...
    if ($stmt->num_rows > 0) {
        $stmt->bind_result($idNguoiDung, $tenDangNhapDB, $matKhauMaHoa, $vaiTro);
        $stmt->fetch();
